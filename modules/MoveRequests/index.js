...
module.exports.createModule = function (context) {
    ...
    function meshRows(user) {
        ...
        return Object.keys(meshes).map(function (id) {
            var mesh = meshes[id] || {};
            return { id: mesh._id || id, name: mesh.name || mesh.mname || id };
    ...
    function moveNode(payload, request) {
        ...
        return new Promise(function (resolve, reject) {
            try {
                web.MoveNodeToMesh(payload.nodeId, payload.targetMeshId, request.requester && request.requester.id, function (error) {
                    if (error) reject(new Error(String(error.message || error)));
                    else resolve({ message: "Device moved.", nodeId: payload.nodeId, targetMeshId: payload.targetMeshId });
                });
    ...
    var provider = {
        ...
        getTitle: function (payload) { return "Move " + (payload.nodeName || payload.nodeId || "device"); },
        getSummary: function (payload) { return (payload.sourceMeshName || payload.sourceMeshId || "Current group") + " → " + (payload.targetMeshName || payload.targetMeshId); },
    ...
    return {
        ...
        clientConfig: function () {
            ...
            return {
                ...
                menuEnabled: value.menuEnabled === true,
                toolbar: { refresh: true, clear: true, favorites: false, search: true, manage: false, settings: true }
            };
        },
        getAccess: access,
        initialize: function () {
        ...
        apiGet: function (asset, req, user) {
            ...
            if (asset === "meshes") return { ok: true, meshes: meshRows(user) };
            if (asset === "requests") {
                var q = Object.assign({}, req && req.query || {}, { type: "moverequests" });
                return context.approval.list(user, q).then(function (value) { value.ok = true; return value; });
            }
            if (asset === "settings") return { ok: true, settings: context.settings.read().modules.moverequests || {} };
            throw new Error("Unknown Move Requests action.");
        },
        apiPost: function (asset, req, user) {
            var value = req && req.body || {};
            if (asset === "submit") return context.approval.submit("moverequests", user, value, value.note)
            ...
            if (asset === "settings") {
                ...
                return context.settings.update(function (current) {
                    ...
                    current.modules.moverequests.menuEnabled = value.menuEnabled === true;
                    return current;
