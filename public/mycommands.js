(function () {
    ...
    function find(node, path) {
        ...
        for (var i = 0; i < children.length; i++) {
            var result = find(children[i], path);
    ...
    function renderResults(shell) {
        return shell.api("results", {
            ...
            (result.rows || []).forEach(function (row) {
                ...
                if (row.output) {
                    var pre = shell.element("pre", "mc-shared-output", row.output);
    ...
    function openCustom(shell) {
        ...
        shell.post("execute", {
            ...
            shell.state.tab = "results";
            shell.state.page.tabs.select("results", true);
        ...
        render: function (shell) {
            ...
            if (shell.state.tab === "commands") {
                shell.state.page.details.appendChild(shell.card(
                    "Commands",
                    "Use Custom command or select a script. Command actions are placed on the right side of the shared toolbar."
                ));
                return;
            }
            ...
            return shell.api("scripts").then(function (result) {
                ...
                if (shell.state.search) {
                    children = window.MyCompanyCore.flattenScripts(folder)
                        .filter(function (item) {
                            return [item.label, item.name, item.description]
                                .join(" ")
                                .toLowerCase()
                                .indexOf(shell.state.search.toLowerCase()) >= 0;
                        });
                }
                ...
                shell.nav(
                    ...
                    function (item) {
                        if (item.source.type === "directory") {
                            selectedRoot = item.source.path;
                            shell.render();
                            return;
                        }
                        shell.api("script", { path: item.source.path })
                            .then(function (result) {
                                shell.state.page.details.innerHTML = "";
                                shell.state.page.details.appendChild(shell.card(
                                    result.script.label,
                                    result.script.description || result.script.path
                                ));
                                ...
                                run.onclick = function () {
                                    shell.post("execute", {
                                        ...
                                        approvalLevels: result.script.approvalLevels || []
                                    }).then(function () {
                                        shell.state.tab = "results";
                                        shell.state.page.tabs.select("results", true);
                                    }).catch(function (error) {
